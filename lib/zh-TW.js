...
(function() {
  var Moment, date2object, dateExpression2moment, dayTime2moment, expressions, number2integer, time2object, words;
  ...
  dayTime2moment = function(daytime, timezone, moment) {
    ...
    if (match = daytime.match(RegExp("(?:(?:(" + words.dayPeriods + ")?" + words.interjection + "?(" + words.time + ") ?(" + words.dayPeriods + ")?)|(" + words.dayPeriods + "))"))) {
      if (period = match[4]) {
        ...
        moment.endTime = (moment != null ? moment.endTime : void 0) || (moment && Moment(moment).tz(timezone)) || Moment().tz(timezone);
        moment.minute(0);
        moment.second(0);
  ...
  dateExpression2moment = function(dateExp, timezone) {
    var dateObj, day, match, moment, _ref, _ref1;
    if (!dateExp) {
      return null;
    }
  ...
  expressions.push(function(text, timezone) {
    var date, endMoment, eventName, fullDay, location, match, moment;
    if (match = text.match(RegExp("^(?:([^" + words.separators + "]+) ?" + words.event_postfix + ")? ?(" + words.dateExpression + ") ?(" + words.dayTime + ")? ?(?:" + words.to + " ?(" + words.dateExpression + ") ?(" + words.dayTime + ")? ?) ?(?:" + words.at + " ?([^" + words.separators + "]+))? ?(?:" + words.event_prefix + words.interjection + "?([^" + words.separators + "]+))?$"))) {
      moment = dateExpression2moment(match[2], timezone) || Moment().tz(timezone);
      if (match[3]) {
      ...
      if (match[4]) {
        endMoment = dateExpression2moment(match[4], timezone);
      }
      if (endMoment && match[5]) {
        endMoment = dayTime2moment(match[5], timezone, endMoment);
      } else if (endMoment && !match[5]) {
        endMoment.hour(23);
        endMoment.minute(59);
        endMoment.second(59);
      }
  ...
  expressions.push(function(text, timezone) {
    var date, endMoment, eventName, location, match, moment;
    if (match = text.match(RegExp("^(?:([^" + words.separators + "]+) ?" + words.event_postfix + ")? ?(?:(" + words.dateExpression + ")) ?(" + words.dayTime + ") ?(?:" + words.to + " ?(" + words.dayTime + ") ?)? ?(?:" + words.at + " ?([^" + words.separators + "]+))? ?(?:" + words.event_prefix + words.interjection + "?([^" + words.separators + "]+))?$"))) {
      ...
      if (match[4]) {
        endMoment = dayTime2moment(match[4], timezone, moment);
      }
      location = match[5];
